...
export function CustomerMergeDialog({
  ...
  const fieldResolutions = useMemo(
    () => merge.preview ? buildMergeFieldResolutions(merge.preview, drafts) : { invalidScalarFields: [], invalidMetadataKeys: [] },
      ...
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-label={t("manager.customerMerge.title")} className="flex max-h-[96dvh] w-full flex-col overflow-hidden rounded-t-[1.7rem] border border-blush/24 bg-card shadow-soft sm:max-h-[92vh] sm:max-w-3xl sm:rounded-[1.7rem]" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          {merge.phase === "completion_unknown" ? <UnknownCompletion onRetry={merge.retrySameRequest} /> : <>
            ...
            {(merge.phase === "reviewing" || merge.phase === "merging") && merge.preview && <PreviewReview preview={merge.preview} drafts={drafts} onDraft={updateDraft} t={t} showValidation={showValidation} invalidScalarFields={fieldResolutions.invalidScalarFields ?? []} invalidMetadataKeys={fieldResolutions.invalidMetadataKeys} />}
            ...
            {merge.phase === "confirming" && merge.preview && <section className="mt-6 rounded-xl border border-blush-strong/35 bg-blush/10 p-4"><h3 className="font-serif text-2xl">{t("manager.customerMerge.confirmation")}</h3><p className="mt-2 text-sm leading-6 text-foreground/68">{t("manager.customerMerge.confirmationBody")}</p><CommitmentSummary preview={merge.preview} drafts={drafts} t={t} /><div className="mt-4 flex flex-wrap gap-2"><Button type="button" variant="outline" className="rounded-full" onClick={() => merge.setSurvivorCustomerId(merge.survivorCustomerId)}>{t("manager.customerMerge.back")}</Button><Button type="button" className="rounded-full" disabled={!fieldResolutions.value || mutationDenied} onClick={() => fieldResolutions.value && void merge.submit(fieldResolutions.value)}>{t("manager.customerMerge.submit")}</Button></div></section>}
...
function PreviewReview({ preview, drafts, onDraft, t, showValidation, invalidScalarFields, invalidMetadataKeys }: { preview: NonNullable<ReturnType<typeof useCustomerMerge>["preview"]>; drafts: MergeResolutionDrafts; onDraft: (key: string, next: Partial<MergeResolutionDraft>, metadata?: boolean) => void; t: (key: string, options?: Record<string, unknown>) => string; showValidation: boolean; invalidScalarFields: string[]; invalidMetadataKeys: string[] }) {
...
function Consequences({ preview, t }: { preview: NonNullable<ReturnType<typeof useCustomerMerge>["preview"]>; t: (key: string, options?: Record<string, unknown>) => string }) {
  ...
  const movementKeys = ["membershipGrants", "membershipLedger", "classRegistrations", "classParticipants"] as const;
  return <section className="grid gap-4 sm:grid-cols-2"><Summary title={t("manager.customerMerge.membership")}><p>{membershipKey ? t(`manager.customerMerge.membershipOutcomes.${membershipKey}`) : t("manager.customerMerge.genericOutcome")}</p><GrantSummary label={t("manager.customerMerge.sourceGrant")} grant={preview.membershipResolution.sourceGrant} t={t} /><GrantSummary label={t("manager.customerMerge.survivorGrant")} grant={preview.membershipResolution.survivorGrant} t={t} /></Summary><Summary title={t("manager.customerMerge.registrations")}><p>{t("manager.customerMerge.moved", { count: preview.registrations.movedCount })} · {t("manager.customerMerge.collisions", { count: preview.registrations.collisionCount })}</p>{preview.registrations.samples.map((sample, index) => <p key={index} className="mt-2 text-sm text-foreground/62">{knownMergeOutcomeKey(sample.rule) ? t(`manager.customerMerge.registrationRules.${sample.rule}`) : t("manager.customerMerge.genericOutcome")}{sample.stockRestoration ? ` · ${t("manager.customerMerge.stockRestored", { count: sample.stockRestoration.stockRestored })}` : ""}</p>)}{preview.registrations.samplesTruncated && <p className="mt-2 text-sm text-foreground/62">{t("manager.customerMerge.samplesTruncated")}</p>}</Summary><Summary title={t("manager.customerMerge.participants")}><p>{t("manager.customerMerge.moved", { count: preview.participants.movedCount })} · {t("manager.customerMerge.collisions", { count: preview.participants.collisionCount })}</p>{preview.participants.samples.map((sample, index) => <dl key={index} className="mt-2 grid gap-1 text-sm text-foreground/62"><OutcomeRow label={t("manager.customerMerge.participant.attendance")} value={participantOutcomeLabel("participantAttendance", sample.finalAttendance, t)} /><OutcomeRow label={t("manager.customerMerge.participant.kind")} value={participantOutcomeLabel("participantKinds", sample.finalKind, t)} /><OutcomeRow label={t("manager.customerMerge.participant.attendanceRule")} value={participantOutcomeLabel("participantAttendanceRules", sample.attendanceRule, t)} /><OutcomeRow label={t("manager.customerMerge.participant.representationRule")} value={participantOutcomeLabel("participantRepresentationRules", sample.representationRule, t)} /></dl>)}{preview.participants.samplesTruncated && <p className="mt-2 text-sm text-foreground/62">{t("manager.customerMerge.samplesTruncated")}</p>}</Summary><Summary title={t("manager.customerMerge.movements")}><dl className="mt-2 grid grid-cols-2 gap-2 text-sm">{movementKeys.map((key) => <div key={key}><dt className="break-words text-foreground/52">{t(`manager.customerMerge.movementCounts.${key}`)}</dt><dd className="font-medium">{preview.movementCounts[key]}</dd></div>)}</dl></Summary></section>;
}

function GrantSummary({ label, grant, t }: { label: string; grant: NonNullable<ReturnType<typeof useCustomerMerge>["preview"]>["membershipResolution"]["sourceGrant"]; t: (key: string, options?: Record<string, unknown>) => string }) {
  if (!grant) return null;
  return <dl className="mt-3 grid gap-1 text-sm text-foreground/62"><OutcomeRow label={label} value={grant.membershipType.name} /><OutcomeRow label={t("manager.customerMerge.membershipStatus")} value={t(`manager.customers.membershipStatus.${grant.status}`)} /><OutcomeRow label={t("manager.customerMerge.membershipStock")} value={grant.remainingStock === null ? t("manager.customerMerge.membershipNoStock") : t("manager.customerMerge.membershipStock", { count: grant.remainingStock })} /></dl>;
}

function OutcomeRow({ label, value }: { label: string; value: string }) { return <div><dt className="inline text-foreground/52">{label}: </dt><dd className="inline">{value}</dd></div>; }
function participantOutcomeLabel(category: "participantAttendance" | "participantKinds" | "participantAttendanceRules" | "participantRepresentationRules", value: string, t: (key: string) => string) { const known = { participantAttendance: new Set(["present", "absent"]), participantKinds: new Set(["registered", "walk_in", "trial"]), participantAttendanceRules: new Set(["present_wins"]), participantRepresentationRules: new Set(["survivor_wins"]) }; return known[category].has(value) ? t(`manager.customerMerge.${category}.${value}`) : t("manager.customerMerge.participant.reported"); }

function CommitmentSummary({ preview, drafts, t }: { preview: NonNullable<ReturnType<typeof useCustomerMerge>["preview"]>; drafts: MergeResolutionDrafts; t: (key: string, options?: Record<string, unknown>) => string }) {
  const scalarRows = [["displayName", preview.fieldComparisons.displayName], ["contactEmail", preview.fieldComparisons.contactEmail], ["phoneNumber", preview.fieldComparisons.phoneNumber]] as const;
  return <div className="mt-4 grid gap-4"><IdentitySummary label={t("manager.customerMerge.source")} customer={preview.source} /><IdentitySummary label={t("manager.customerMerge.survivor")} customer={preview.survivor} /><Summary title={t("manager.customerMerge.finalChoices")}><dl className="grid gap-2">{scalarRows.map(([field, comparison]) => <OutcomeRow key={field} label={t(`manager.customerMerge.fields.${field}`)} value={finalScalarValue(drafts[field], comparison.sourceValue, comparison.survivorValue, t)} />)}</dl></Summary><Summary title={t("manager.customerMerge.finalMetadata")}><dl className="grid gap-2">{preview.fieldComparisons.metadata.conflicts.map((conflict) => <OutcomeRow key={conflict.key} label={conflict.key} value={`${t("manager.customerMerge.finalSelection", { selection: selectionLabel(drafts.metadata[conflict.key]?.selection, t) })} · ${finalMetadataValue(drafts.metadata[conflict.key], conflict.source.present ? formatMergeJsonValue(conflict.source.value) : t("manager.customerMerge.noValue"), conflict.survivor.present ? formatMergeJsonValue(conflict.survivor.value) : t("manager.customerMerge.noValue"), t)}`} />)}</dl></Summary><Consequences preview={preview} t={t} /><p className="text-sm text-foreground/62">{t("manager.customerMerge.expiry", { date: new Date(preview.expiresAt).toLocaleString() })}</p></div>;
}

function finalScalarValue(draft: MergeResolutionDraft | undefined, source: string | null, survivor: string | null, t: (key: string) => string) { if (draft?.selection === "source") return source ?? t("manager.customerMerge.noValue"); if (draft?.selection === "survivor") return survivor ?? t("manager.customerMerge.noValue"); return draft?.replacement?.trim() || t("manager.customerMerge.noValue"); }
function finalMetadataValue(draft: MergeResolutionDraft | undefined, source: string, survivor: string, t: (key: string) => string) { if (draft?.selection === "source") return source; if (draft?.selection === "survivor") return survivor; return draft?.replacement?.trim() || t("manager.customerMerge.noValue"); }
function selectionLabel(selection: MergeResolutionDraft["selection"] | undefined, t: (key: string) => string) { return selection ? t(`manager.customerMerge.selections.${selection}`) : t("manager.customerMerge.noValue"); }
