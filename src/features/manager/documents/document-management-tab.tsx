...
export function DocumentManagementTab({
  ...
  async function saveDraft() {
    const documentValues = getDocumentValues(form);
  ...
  async function saveAndPublish() {
    ...
    try {
      ...
      if (!draftToPublish || hasUnsavedChanges) {
        const savedDraft = await client.management.productDocuments.saveDraft({
          documentType: mutationSelection.documentType,
          locale: mutationSelection.locale,
          ...documentValues,
          effectiveAt: toEffectiveAt(form.effectiveAt),
        ...
        draftToPublish = savedDraft.draft;
        const nextForm = toForm(savedDraft.draft);
      ...
      const result = await client.management.productDocuments.publishDraft({
        documentType: mutationSelection.documentType,
        locale: mutationSelection.locale,
        expectedDraftRevision: draftToPublish.revision,
  ...
  return (
    <section className="mx-auto w-full max-w-[96rem] rounded-[1.4rem] border border-blush/24 bg-card/78 p-5 shadow-soft sm:p-6">
      ...
      {!canManageDocuments ? (
        ...
        <div className="mt-5 grid gap-5">
          ...
          <section className="rounded-xl border border-blush/24 bg-background/40 p-4 sm:p-5">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2" role="list" aria-label={t("manager.documents.documentType")}>
                  ...
                  <div key={document.type} role="listitem">
                    <button
                      ...
                      className={[
                        ...
                        selected ? "border-blush-strong/70 bg-blush/16" : "border-blush/20 bg-card/38 hover:border-blush/44 hover:bg-card/60",
                      ].join(" ")}
                      disabled={loadStatus === "loading" || isMutating}
                      onClick={() => requestSelection({ ...selectionRef.current, documentType: document.type })}
                    >
                      <span className="grid size-9 shrink-0 place-items-center rounded-full border border-blush/30 text-blush-strong">
                        <Icon className="size-4" aria-hidden="true" />
                      </span>
                      <span className="min-w-0">
                        <span className="block font-serif text-xl text-foreground">{t(document.labelKey)}</span>
                        <span className="mt-0.5 block text-xs leading-5 text-foreground/62">{t(document.descriptionKey)}</span>
                      </span>
                    </button>
                  </div>
                );
          ...
          {isCurrentWorkspace && (
            <div className="grid gap-5">
              <div className={[
                ...
                isEditing && activeVersion ? "xl:grid-cols-[minmax(0,3fr)_minmax(20rem,2fr)]" : "",
              ].join(" ")}>
                {isEditing && (
                  <section className="overflow-hidden rounded-xl border border-blush-strong/40 bg-blush/8">
                    <div className="flex flex-col gap-3 border-b border-blush/18 px-5 py-4 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                      ...
                      <p className="self-start rounded-full bg-background/64 px-3 py-1 text-xs font-semibold text-foreground/76">
                    ...
                    <div className="grid gap-4 p-5">
                      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_14rem]">
                        <label className="grid gap-2 text-sm font-semibold text-foreground/76">
                          ...
                          <input
                            value={form.title}
                            className="h-11 min-w-0 rounded-xl border border-blush/24 bg-card/70 px-3 text-sm text-foreground outline-none focus:border-blush-strong"
                            onChange={(event) => {
                      ...
                      <label className="grid gap-2 text-sm font-semibold text-foreground/76">
                        ...
                        <textarea
                          ...
                          rows={20}
                          className="min-h-[32rem] resize-y rounded-xl border border-blush/24 bg-card/70 px-4 py-4 font-mono text-sm leading-7 text-foreground outline-none focus:border-blush-strong xl:min-h-[38rem]"
                          onChange={(event) => {
                    ...
                    <footer className="border-t border-blush/18 bg-background/36 px-5 py-4">
                      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-foreground/58">
                        <span>{t("manager.documents.documentStats", { characters: form.contentMarkdown.length, words: wordCount })}</span>
                        <span>{hasUnsavedChanges ? t("manager.documents.unsavedDraft") : draft ? t("manager.documents.draftSaved") : ""}</span>
                      </div>
                      <div className="mt-3 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <Button type="button" size="sm" variant="ghost" className="self-start rounded-full text-foreground/68" disabled={isMutating || !hasUnsavedChanges} onClick={cancelLocalChanges}>
                          <X className="size-4" aria-hidden="true" />
                          {t("manager.documents.cancelChanges")}
                        </Button>
                        <div className="flex flex-col gap-2 sm:flex-row">
                          <Button type="button" variant="outline" className="w-full rounded-full border-blush-strong/60 bg-card/90 text-foreground hover:bg-blush/16 sm:w-auto" disabled={isMutating || Boolean(draft && !hasUnsavedChanges)} onClick={() => void saveDraft()}>
                            {mutationStatus === "saving" ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : <Save className="size-4" aria-hidden="true" />}
                            {t("manager.documents.saveAsDraft")}
                          </Button>
                          <Button type="button" className="w-full rounded-full px-5 sm:w-auto" disabled={isMutating} onClick={() => void saveAndPublish()}>
                            {mutationStatus === "publishing" ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : <Send className="size-4" aria-hidden="true" />}
                            {t("manager.documents.saveAndPublish")}
                          </Button>
                        </div>
                      </div>
                    </footer>
                ...
                {(!isEditing || activeVersion) && (
                  <section className="overflow-hidden rounded-xl border border-blush/24 bg-background/34">
                    <div className="flex flex-wrap items-start justify-between gap-3 border-b border-blush/18 px-5 py-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm leading-6 text-foreground/62">
                          {activeVersion ? t("manager.documents.liveMessage") : t("manager.documents.notLiveMessage")}
                        </p>
                      </div>
                      {activeVersion && (
                    ...
                    {currentLiveVersion ? (
                      ...
                      <p className="px-5 py-6 text-sm leading-6 text-foreground/62">
                        {activeVersion ? t("manager.documents.livePreviewUnavailable") : t("manager.documents.notLiveMessage")}
                      </p>
                    )}
                    <div className="flex flex-wrap gap-2 border-t border-blush/18 px-5 py-4">
                      {currentLiveVersion && !draft && (
                        <Button type="button" className="rounded-full" disabled={isMutating} onClick={() => void restoreVersionAsDraft(currentLiveVersion)}>
                      ...
                      {!activeVersion && !draft && !editorOpen && (
                        <Button type="button" className="rounded-full" disabled={isMutating} onClick={() => setEditorOpen(true)}>
              ...
              <details className="rounded-xl border border-blush/20 bg-background/30 p-4">
                <summary className="cursor-pointer text-foreground">
                  <span className="ms-2 inline-flex items-center gap-2">
                    ...
                    <span className="font-serif text-2xl">{t("manager.documents.history")}</span>
                    <span className="rounded-full bg-blush/14 px-2 py-0.5 text-xs font-semibold text-foreground/62">
                      {timelineVersions.length === 1
                        ? t("manager.documents.versionCountOne")
                        : t("manager.documents.versionsCount", { count: timelineVersions.length })}
                    </span>
                  </span>
                </summary>
                <p className="mt-3 text-sm leading-6 text-foreground/62">{t("manager.documents.olderVersionsHint")}</p>
                      ...
                      return (
                        <button
                          ...
                          className={[
                            ...
                            isPreviewed ? "border-blush-strong/70 bg-blush/14" : "border-blush/20 bg-card/42 hover:border-blush/44 hover:bg-card/64",
                          ].join(" ")}
                          disabled={isMutating}
                        ...
                        >
                          ...
                          <span className="mt-2 block text-sm leading-5 text-foreground/70">{version.title}</span>
                          <span className="mt-2 block text-xs text-foreground/52">{formatPublishedDate(version.published_at, selection.locale)}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                {previewedVersion && (
                  <section className="mt-5 overflow-hidden rounded-xl border border-blush/24 bg-card/46">
                    <div className="flex flex-wrap items-start justify-between gap-3 border-b border-blush/18 px-5 py-4">
                      <div>
              ...
              {(activeVersion || draft) && (
                <details className="rounded-xl border border-blush/20 bg-background/30 p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 border-t border-blush/18 pt-4">
                    {draft && (
